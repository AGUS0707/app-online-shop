...
import {ModalBody, Modal as ModalContent, Button, Spinner} from "reactstrap";
...
function Basket(props) {
    ...
    useEffect(()=>{
        ...
        axios.get(API_PATH+'values').then(response=>{
           setValueListGet(response.data);
        });
    }, []);

    const [productIdList, setProductIdList]=useState([]);
    const [valueListGet, setValueListGet]=useState([]);
    ...
    function deleteProductCard() {
        axios.post(API_PATH+'delcart' ,{id:id}, {headers:{"Authorization": "Bearer " + Cookies.get('jwt')}}).then((response)=>{
            ...
            axios.post(API_PATH+'cart', {id:props.userReducer.userObject.id}, {headers:{"Authorization": "Bearer " + Cookies.get('jwt')}}).then((response1)=>{
                ...
                props.set_state({countCart:response1.data.length})
            })
        })
    }
    function deleteProductCardMobile(del_id) {
        axios.post(API_PATH+'delcart' ,{id:del_id}, {headers:{"Authorization": "Bearer " + Cookies.get('jwt')}}).then((response)=>{
            axios.post(API_PATH+'cart', {id:props.userReducer.userObject.id}, {headers:{"Authorization": "Bearer " + Cookies.get('jwt')}}).then((response1)=>{
                props.shoppingCardListFunction(response1.data);
                // props.shoppingCardCountFunction(response1.data.length);
                props.set_state({countCart:response1.data.length})
    ...
    function checkChange(e){
        let inputList=window.innerWidth>576 ? document.querySelectorAll('.input'):document.querySelectorAll('.input1');
        console.log(e.currentTarget.cloneNode(true));
        if (e.target.id==="bosh"){
    ...
    function purchaseFunctionLocal() {
        if (props.userCheckReducer.userCheck){
            if (productIdList.length>0){
                ...
                props.shoppingCardListReducer.shoppingCardList.forEach((item)=>{
                    productIdList.forEach((item2)=>{
                        if (parseFloat(item2)===item.id){
                            let detailList=[];
                            let valueList=[];
                            let newObject;
                            item.detail.forEach((detail)=>{
                                detailList=detailList.concat( detail.detail_id.toString())
                            });
                            item.value.forEach((value)=>{
                                valueList=valueList.concat( value.value_id.toString())
                            });
                            newObject={
                                ...item,
                                detail:detailList,
                                ...
                                <div className="shoppingCardLeft">
                                    <div className="shoppingCardLeftHeader">
                                        ...
                                        <div className="shoppingAllSelect d-flex align-items-center">
                                            ...
                                            <span className="checkText mt-2">Hammasini belgilash</span>
                                        </div>
                                        <div className="cartTextNone d-none mt-1">
                                            Savatcha
                                        </div>
                                    </div>
                                        ...
                                        return  <div className="shoppingCardList">
                                            <div className="shoppingCardItem">
                                                <div
                                                    ...
                                                    {/*</div>*/}
                                                    <div className="shoppingProductCheck d-none">
                                                        <input onChange={checkChange} id={item.id} type="checkbox" className="input1"/>
                                                    </div>
                                                    <div className="deleteIcon d-none" onClick={()=>deleteProductCardMobile(item.id)}>
                                                        <div className="d-flex justify-content-center align-items-center">
                                                            <img src="/images/cancel.svg" alt="no images"/>
                                                        </div>
                                                    </div>
                                                </div>
                                                <hr/>
                                                <div className="shoppingCardItemMain">
                                                    ...
                                                    <div className="shoppingCardItemMainCenter">
                                                        ...
                                                        <Link  to="/" className={`shoppingProductLink ${window.innerWidth<576? "d-none": ""}`}><p>{parse(item.description_uz)}</p></Link>
