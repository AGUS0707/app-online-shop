...
function ProductForm(props) {
    ...
    useEffect(( )=>{
        ...
        axios.post(API_PATH+"slproduct", {user_id:props.userReducer.userObject.id}, {headers:{"Authorization": "Bearer " + Cookies.get('jwt')}}).then((res)=>{
            res.data.forEach((item)=>{
                if (item.id===JSON.parse(localStorage.getItem('sellerEditProductId'))){
                    props.updateProduct(item);
                    saleCheckFunction(item);
                }
            });
        });
        valueChangeListChecked();
        props.editValueChangeList(JSON.parse(localStorage.getItem('valueChangeList')));
        function saleCheckFunction(item) {
            if (item.is_sale==="1"){    // chegirmalar oynasi chiqishini tekshirdim
                setSaleCheck(true);
            }else{
                setSaleCheck(false);
            }
    ...
    function valueChangeListChecked() {
        let a=document.querySelectorAll('.ggg');
        a.forEach((item)=>{
            props.valueChangeListReducer.valueList.forEach((item2)=>{
                if (item.name===item2){
    ...
    const imgListFunction=()=> {
        if (imgCheck){
            ...
            return <div className="productImgListItem">
                <img src="https://attachments.developer.intuit.com/appcard-e478611f-fe83-48aa-b3e4-a07524032c46.png"/>
            </div>
        }
    };
    const addProduct=()=> {
        ...
        if (props.valueChangeListReducer.valueList.length>0&&imgList.length>0&&product.product_uz.length>0&&product.product_ru.length>0&&product.price.length>0&&product.amount.length>0&&product.min_order.length>0&&product.description_uz.length>0&&product.description_ru.length>0&&product.category_id.length>0&&product.brand_id.length>0&&product.is_sale.length>0&&product.serial_number.length>0&&product.xarakteristika.length>0&&imgList.length>0){
            ...
            axios.post(API_PATH+'upproduct', formDate, {headers:{"Authorization": "Bearer " + Cookies.get('jwt')}})
                .then((response)=>{
                        ...
                        <div className={`menu3 `}>
                            ...
                            <div className={`form-group ${brand_idCheck? "menusValid": " "}`}>
                                <div className="row">
                                    ...
                                    <div className="col-sm-10">
                                        <select  className="form-control"  value={product.brand_id}  name="brand_id" onChange={handleInputChange}>
                                            <option value="">Choose</option>
                                            {brandList.map((item)=>{
                                                    ...
                                                    <div className="col-8 d-flex align-items-center">
                                                        {
                                                            props.valuesListReducer.valuesList.map((item2, index)=>{
                                                                return item.id == item2.detail_id ?  <div className="input-group d-flex align-items-center aaa">
                                                                    ...
                                                                    <input  type="checkbox" id={item2.detail_id} name={item2.id} className="form-check-inline ggg" onClick={(e)=>handleInputValueChange(e)}/>
                                                                </div> : ""
                                                            })
                                                        }
                                                    </div>
