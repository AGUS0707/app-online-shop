...
function ProductForm(props) {
    ...
    useEffect(()=>{
        axios.get(API_PATH+'category').then((response)=>{
    ...
    function addProduct() {
        ...
        if (imgList.length>0&&product.product_uz.length>0&&product.product_ru.length>0&&product.price.length>0&&product.amount>0&&product.min_order>0&&name_uz.length>0&&name_ru.length>0&&product.category_id!==undefined&&product.brand_id!==undefined&&product.is_sale!==undefined&&product.sale_price>0&&product.serial_number>0&&product.xarakteristika.length>0&&imgList.length>0){
            const formDate=new FormData();
            formDate.append('id', props.userReducer.userObject.id);
            formDate.append('product_uz', product.name_uz);
            formDate.append('product_ru', product.name_ru);
